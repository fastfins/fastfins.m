function [state_f, kappa_f, forcing_f, bottom_flux, bnd_funs, bc_types, bc_funs] = ref_soln2d_tensor(plot_on)
%ref_soln2d
% 
...
b1(x1, x2) = cos(pi*x1) * sin(pi*x2 - pi/2);
b2(x1, x2) = cos(pi*x1 - pi/2) * sin(pi*x2);
...
% cross term
dc_x1(x1, x2) = b1*(ds_x1*b1+ds_x2*b2);
dc_x2(x1, x2) = b2*(ds_x1*b1+ds_x2*b2);

% forcing
forcing(x1, x2) = - (diff(kappa*ds_x1, x1) + diff(kappa*ds_x2, x2)) ...
    - (diff(dc_x1, x1) + diff(dc_x2, x2));

% flux
left_flux  (x1, x2) = - kappa(0, x2)*ds_x1(0, x2) - dc_x1(0, x2);
right_flux (x1, x2) = kappa(1, x2)*ds_x1(1, x2) - dc_x1(1, x2);
bottom_flux(x1, x2) = - kappa(x1, 0)*ds_x2(x1, 0) - dc_x2(x1, 0);
top_flux   (x1, x2) = kappa(x1, 1)*ds_x2(x1, 1) - dc_x2(x1, 1);
...
kappa_f     = {matlabFunction(kappa), matlabFunction(b1), matlabFunction(b2)};
