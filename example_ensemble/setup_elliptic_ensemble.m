...
% for the MCMC
% need to change the number of steps
mcmc_opt = mcmc_options('nstep', 1E3, 'sbatch', 1, 'lis_flag', false, 'sigma', -1);
hess = @(v) eigen_PPFisher(model, obs, prior, v, 1E-2, obs.n_data);
tic;
out_hmala = hmala(mlpost, hess, vmap, mcmc_opt);
toc
% out_hmala.samples gives the posterior samples in v

% prior samples, from i.i.d Gaussian
n = 1E2;
v = randn(prior.dof, n);
