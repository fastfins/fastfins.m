function [state_f, kappa_f, forcing_f, bottom_flux, bnd_funs, bc_types, bc_funs] = ref_soln2d_vector(plot_on)
%ref_soln2d
% 
...
kappa1(x1, x2) = 2 + cos(2*pi*x1 + pi/4) * sin(2*pi*x2 + pi/4);
kappa2(x1, x2) = 2 + cos(2*pi*x1 - pi/4) * sin(2*pi*x2 - pi/2);
...
% forcing
forcing(x1, x2) = - (diff(kappa1*ds_x1, x1) + diff(kappa2*ds_x2, x2));

% flux
left_flux  (x1, x2) = -kappa1(0, x2)*ds_x1(0, x2);
right_flux (x1, x2) = kappa1(1, x2)*ds_x1(1, x2);
bottom_flux(x1, x2) = -kappa2(x1, 0)*ds_x2(x1, 0);
top_flux   (x1, x2) = kappa2(x1, 1)*ds_x2(x1, 1);
...
kappa_f     = {matlabFunction(kappa1), matlabFunction(kappa2)};
