function true_x = load_test_image(prior, type, base, range)
%INIT_TEST_IMAGE
%
...
switch type
    case {'CF'}
        true_x = conduct_cf(prior.mesh, base, range);
    case {'Inclusion'}
        true_x = conduct_inclusion(prior.mesh, base, range);
    case {'Prior'}
...
r     = randn(prior.dof, 1);
cond  = matvec_prior_L(prior, r) + prior.mean_u;
...
centers     = [0.3, 0.4; 0.7 0.8; 0.3, 0.8; 0.8 0.2];
r           = [0.1, 0.1, 0.1, 0.1];
type        = [2, 2, 2, 2];
