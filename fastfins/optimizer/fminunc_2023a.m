function [x,FVAL,EXITFLAG,OUTPUT,GRAD,HESSIAN] = fminunc_2023a(FUN,x,options,varargin)
...
if ~isempty( isoptimargdbl('FMINUNC', {'f','GRAD'}, f, GRAD) )
...
if diagnostics
   ...
   if ~hessflag
       if strcmpi(output.algorithm, medium)
           if iscell(options.HessUpdate)
               hessflag = options.HessUpdate{1};
           elseif contains(options.HessUpdate, 'bfgs')
               hessflag = options.HessUpdate;
           else
               hessflag = getString(message('optimlib:diagnose:QuasiNewton'));
           end
       else
